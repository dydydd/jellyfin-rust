...
use jellyfin_providers::media_info::{AudioResolveRequest, AudioResolver, MediaFileSystemEntry};
...
fn resolver() -> AudioResolver<'static, TestLocalizationManager> {
    static LOCALIZATION_MANAGER: TestLocalizationManager = TestLocalizationManager;
    AudioResolver::new(NamingOptions::default(), &LOCALIZATION_MANAGER)
}

fn resolve(
    file: &str,
    metadata_directory: bool,
) -> Vec<jellyfin_providers::media_info::ResolvedAudioStream> {
    ...
    resolver().resolve(AudioResolveRequest {
...
#[test]
fn mixed_filenames_official_matrix_picks_audio() {
    for (file, metadata_directory, matches) in [
        ("My.Video.srt", false, false),
        ("My.Video.mp3", false, true),
        ("My.Video.srt", true, false),
        ("My.Video.mp3", true, true),
    ] {
        let streams = resolve(file, metadata_directory);
        assert_eq!(streams.len(), usize::from(matches));
        if matches {
            assert_eq!(streams[0].stream.stream_type, MediaStreamType::Audio);
            assert_eq!(streams[0].mime_type, "audio/mpeg");
        }
    }
}
...
fn rejects_remote_directory_and_missing_folder_sources() {
    let candidate = [MediaFileSystemEntry::file("Test Data/Video/My.Video.mp3")];
    for (protocol, media_is_directory, directory_exists, media_path) in [
        ...
        let streams = resolver().resolve(AudioResolveRequest {
            media_path,
            protocol,
            media_is_directory,
...
fn filters_directories_source_file_stream_files_and_unresolved_items() {
    let entries = [
        MediaFileSystemEntry::directory("Test Data/Video/My.Video.mp3"),
        ...
        MediaFileSystemEntry::file("Test Data/Video/My.Video2.mp3"),
        MediaFileSystemEntry::file("Test Data/Video/My.Video Sequel.mp3"),
        MediaFileSystemEntry::file("Test Data/Video/Some.Other.Video.mp3"),
    ];
    let ignored_metadata = [MediaFileSystemEntry::file("library/00/My.Video.valid.mp3")];
    let streams = resolver().resolve(AudioResolveRequest {
        media_path: VIDEO_PATH,
        protocol: MediaProtocol::File,
        media_is_directory: false,
...
fn handles_extensions_tokens_and_stream_indices() {
    let entries = [
        MediaFileSystemEntry::file("Test Data/Video/My.Video.English.default.MP3"),
        MediaFileSystemEntry::file("Test Data/Video/My.Video.Commentary.fr.flac"),
        MediaFileSystemEntry::file("Test Data/Video/my.video.aac"),
    ];
    let streams = resolver().resolve(AudioResolveRequest {
        media_path: VIDEO_PATH,
        protocol: MediaProtocol::File,
    ...
    assert_eq!(streams[1].mime_type, "audio/flac");
    assert_eq!(streams[2].stream.index, 6);
    assert_eq!(
        streams[2].stream.path.as_deref(),
        Some(entries[2].path.as_str())
    );
...
fn handles_windows_paths_and_metadata_directory_boundaries() {
    let directory_entries = [MediaFileSystemEntry::file(r"C:\Videos\My.Video.en.mp3")];
    let metadata_entries = [
        MediaFileSystemEntry::file(r"D:\Metadata\My.Video.fr.mp3"),
        MediaFileSystemEntry::file(r"D:\Metadata\My.VideoExtra.mp3"),
    ];
    let streams = resolver().resolve(AudioResolveRequest {
...
#[test]
fn honors_configured_audio_extensions_and_delimiters() {
    static LOCALIZATION_MANAGER: TestLocalizationManager = TestLocalizationManager;
    let options = NamingOptions {
        audio_file_extensions: vec![".audiox".to_owned()],
        media_flag_delimiters: vec!['-'],
        ..NamingOptions::default()
    };
    let resolver = AudioResolver::new(options, &LOCALIZATION_MANAGER);
    let entries = [
        MediaFileSystemEntry::file("Test Data/Video/My.Video-en.audiox"),
        MediaFileSystemEntry::file("Test Data/Video/My.Video.en.audiox"),
