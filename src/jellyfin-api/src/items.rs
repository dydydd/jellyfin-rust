...
impl AppState {
    ...
    async fn emby_library_facet(
        ...
        let facet = match facet_name {
            ...
            "VideoCodec" => BaseItemFacet::VideoCodec,
            "SubtitleCodec" => BaseItemFacet::SubtitleCodec,
            "StreamLanguage" => BaseItemFacet::StreamLanguage,
            "Tag" => BaseItemFacet::Tag,
...
async fn page_to_dto_with_fields_and_options(
    ...
    let mut external_urls =
        user_library::external_urls_for_items(state, &page.items, requested_fields).await?;
    ...
    let lyric_item_ids = state
        ...
        .await?;
    let video_item_ids = page
        .items
        .iter()
        .filter(|item| user_library::is_video_base_item(item))
        .map(|item| item.id)
        .collect::<Vec<_>>();
    let subtitle_item_ids = state
        .media_streams
        .item_ids_with_stream_type(&video_item_ids, jellyfin_model::MediaStreamType::Subtitle)
        .await?;
